...
RUN apt-get clean && apt-get update \
    ...
    && /usr/bin/curl -sSo ruby-3.1.2.tar.gz https://cache.ruby-lang.org/pub/ruby/3.1/ruby-3.1.2.tar.gz | bash -s tar -xvf ruby-3.1.2.tar.gz \
    && bash -s ls -la  #bash -s ./configure
    #&& bash -s cd ruby-3.1.2 && bash -s ./configure && bash -s make && bash -s make install
